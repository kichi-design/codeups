...
jQuery(function ($) { // この中であればWordpressでも「$」が使用可能になる
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const submitButton = document.querySelector('.contact-form__button [type="submit"]');
    
        if (submitButton) {
            submitButton.addEventListener('click', function (event) {
                event.preventDefault(); // フォームの自動送信を防ぐ
                let isValid = true;
                const formItems = document.querySelectorAll('.contact-form__item');
                ...
                formItems.forEach(function (item) {
                    ...
                    if (input && input.getAttribute('type') !== 'checkbox' && input.getAttribute('type') !== 'radio') {
                        ...
                        if (!document.querySelector(`input[name="${name}"]:checked`)) {
                            const checkboxGroup = item.querySelector('.contact-form__checkbox');
                            checkboxGroup.classList.add('error');
                            isValid = false;
                        } else {
                            const checkboxGroup = item.querySelector('.contact-form__checkbox');
