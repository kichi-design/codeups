...
function GethashID (hashIDName){
	if(hashIDName){
		//タブ設定
		$('.page-campaign-tab__items li').find('a').each(function() { //タブ内のaタグ全てを取得
			var idName = $(this).attr('href'); //タブ内のaタグのリンク名（例）#lunchの値を取得	
			if(idName == hashIDName){ //リンク元の指定されたURLのハッシュタグ（例）http://example.com/#lunch←この#の値とタブ内のリンク名（例）#lunchが同じかをチェック
				var parentElm = $(this).parent(); //タブ内のaタグの親要素（li）を取得
				$('.page-campaign-tab__items li').removeClass("active"); //タブ内のliについているactiveクラスを取り除き
...
//タブをクリックしたら
$('.page-campaign-tab__items a').on('click', function() {
...
$(window).on('load', function () {
    $('.page-campaign-tab__items li:first-of-type').addClass("active"); //最初のliにactiveクラスを追加
